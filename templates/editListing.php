    ...
    <section>
        <div class="create-listing-page">
            <div class="create-listing-parent-container">
                <div class="create-listing-inner-container">
                    <?php
                        $data = $this -> db -> query("select * from listings where id = ?;", "s", $_COOKIE["item_id"]);
                    ?>
                    <!-- with the login/signup text -->
                    <form class="post-form" action="?command=edit-listing" method="post">
                        <div class="post-details">
                            <h1>Edit My Listing</h1>
                            <br>
                            <!-- title input -->
                            <input type="text" class="title-text-input" placeholder="Title" 
                            value = "<?php echo $data[0]["title"]?>" name="title" required />
                            <!-- price input -->
                            ...
                            <!-- <input type="email" class="email-text-input" placeholder="Condition" name="email"
                                required /> -->
                            <select name="condition" id="condition" title="Condition" value = "<?php echo $data[0]["condition"]?>">
                            ...
                            <select name="category" id="category" title="Category" value = "<?php echo $data[0]["category"]?>">
                            ...
                            <textarea id="description" name="description" rows="4" cols="50" placeholder="Description"
                                 required><?php echo $data[0]["description"]?>
                            </textarea>
                            <input type="submit" class="btn-hover color-3 shrink-on-hover" value="Update">
